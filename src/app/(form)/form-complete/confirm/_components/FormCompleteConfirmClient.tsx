...
import { CheckboxControl, Button } from '@/components/form/field'
import { Tabs } from '@/components/plugins'
import { completeDefaultValuesType, getSchema, getCompleteObject } from '../../_config'
import { CompletePlanConfirmContents, CompleteInputContents } from '../../_components'
import { useForm, FormProvider } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { useSearchParamsContext, useMicroCMSContext, useStripeContext } from '@/features/context'

export const FormCompleteConfirmClient: React.FC = () => {
  /**
   * 初期設定
   */
  // コンテキストから値を取得
   ...
   */
  const onSubmit = async (data: completeDefaultValuesType) => {
    // 送信エラーフラグをリセット
    setIsSubmitError(false)

    // 送信データを作成
    const mergeObject = getCompleteObject({ formValues: data, useMicroCMS, useStripe })
  ...
  const onBack = () => {
    ...
    // 現在のURLを元に前のページのURLを生成
    const backPath = pathname.replace(/\/confirm$/, '/input')
          ...
          <div className="space-y-16">
                ...
                <Tabs
                  tabs={[
                    ...
                    },
                    {
                      ...
                      content: (
                        <div className="mt-10">
                          <CompleteInputContents pageType="confirm" />
                        </div>
                      ),
                      id: 'customer',
                    },
            ...
            <div>
              ...
              <div className="mx-auto mt-10 flex max-w-lg items-center justify-center bg-gray-300 py-5">
                <CheckboxControl
                  nameId="agree"
                  lists={[{ labelContents: '個人情報の取り扱いについて同意する', label: '同意する', value: 'true' }]}
                  error={useFormMethods.formState.errors.agree}
                  {...useFormMethods.register('agree')}
                />
              </div>
            </div>
          </div>
