/**
 * 選択した商品の合計金額を確認画面に表示するコンポーネント
 * @module ConfirmAmount
 */
...
export const ConfirmAmount: React.FC = () => {
  ...
  billingAmount.forEach((billing) => {
    ...
    Object.entries(billing.items).forEach(([key, items]) => {
      ...
      amountContents[key as keyof typeof amountContents] = items.map((item) => {
        return (
          item.price && (
            <dl key={item.price.id} className="py-2 md:flex md:justify-between md:py-3">
              <dt className="text-base font-bold">{item.name}</dt>
    ...
    billing.coupons.forEach((coupon) => {
      ...
      amountContents.coupon = (
        <dl key={coupon.id} className="py-2 md:flex md:justify-between md:py-3">
          <dt className="text-base font-bold">{coupon.name}</dt>
          <dd className="text-right text-base font-bold">
    ...
    if (billing.items.oneTime.length > 0 || billing.items.recurring.length > 0 || billing.coupons.length > 0) {
      content = (
        <div className="mt-4 space-y-3">
          ...
          <div className="border-t border-gray-500 pt-3 md:flex md:justify-between">
            <h3 className="text-xl font-bold">合計金額</h3>
            <p className="text-right text-xl font-bold">{addComma(billing.total)}円（税込）</p>
          </div>
        </div>
      )
    }
  ...
  return (
    <div>
      <HeadingCaption className="mb-8 text-lg md:text-xl">支払い金額</HeadingCaption>
      <div>
        <Tabs
          tabs={tabs}
