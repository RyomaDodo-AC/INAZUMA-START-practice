...
describe('POST', () => {
  ...
  it('contentIdが無い場合エラーが返る', async () => {
    ...
    const result = await response.json()
    expect(result).toEqual({ error: 'contentId is not defined' })
  })
  ...
  it('正常に取得できる', async () => {
    ...
    // client.getListDetailをモック化
    client.getListDetail = jest.fn().mockResolvedValue(obj)
  ...
  it('エラーが発生した場合は500エラーが返る', async () => {
    ...
    // client.getListDetailをモック化
    client.getListDetail = jest.fn().mockRejectedValue(new Error('error'))
