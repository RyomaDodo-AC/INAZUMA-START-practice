...
export const LinkButton: React.FC<LinkButtonProps> = ({ children, color = 'blue', buttonStyle = 'solid', full = true, iconLeft, iconRight, ...linkProps }) => {
  ...
  return (
    ...
    >
      {iconLeft && (
        <span className={`${iconBaseClass} ${iconColorClass} left-4`}>
          {typeof iconLeft === 'string' && iconLeft === 'arrow' ? <Arrow type="angle-left" className={`${iconColorClass}`} /> : iconLeft}
        </span>
      )}
      {children}
      {iconRight && (
        <span className={`${iconBaseClass} ${iconColorClass} right-4`}>
          {typeof iconRight === 'string' && iconRight === 'arrow' ? <Arrow type="angle-right" className={`${iconColorClass}`} /> : iconRight}
        </span>
      )}
    </Link>
  )
