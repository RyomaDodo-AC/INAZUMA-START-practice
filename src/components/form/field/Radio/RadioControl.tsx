...
export interface RadioControlProps extends React.InputHTMLAttributes<HTMLInputElement>, Omit<InputBoxProps, 'children'> {
   ...
   * リスト
   */
  lists: (RadioProps & Omit<CardProps, 'children'>)[]
  /**
   * ラベルのタイプ
   */
  ...
  const radioList = lists.map((list, index) => {
    ...
    switch (radioPosition) {
      ...
      case 'top':
        ...
        switch (labelType) {
          case 'card':
            radioClassNameProps = radioClassNameProps + ' top-4'
            break
          case 'default':
    ...
    switch (labelType) {
      case 'card':
        radioClassNameProps = radioClassNameProps + ' left-2'
    ...
    const Wrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      if (labelType === 'card') {
        return (
          <Card className={`${radioPosition !== 'hidden' && 'py-2 pl-7 pr-5 md:py-3'} ${labelClassName} ${errorLabelClass}`} cardColor={{ active: radioColor }} labelType={true} ribbon={list?.ribbon}>
            {children}
          </Card>
        )
      }
